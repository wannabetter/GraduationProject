<template>
  <div>
    <div style="margin-bottom: 5px;margin-top: 5px;">
      <el-input v-model="Search" style="width: 93%;" prefix-icon="el-icon-search" placeholder="请输入大堂经理姓名">
      </el-input>
      <el-button style="float: right;" type="success" @click="dialogFormVisible =true">增加</el-button>
      <el-dialog title="增加大堂经理" :visible.sync="dialogFormVisible">
        ...
        <div slot="footer" class="dialog-footer">
          <el-button @click="dialogFormVisible = false;$message.info('创建大堂经理失败')">取消</el-button>
    ...
    <el-card v-for="data in Data.slice((currentPage-1)*4,currentPage*4)" :key="data.Account" style="margin-bottom: 5px">
      ...
      <el-popconfirm style="float: right;margin-bottom: 5px;" title="确定删除吗？" @confirm="DeleteL_Manager(data)">
...
<script>
  export default {
    ...
    methods:{
      ...
      DeleteL_Manager(data){
        this.$http.post('http://localhost:3000/api/L_Manager/Delete',
            {Account:data.Account, PassWord:data.PassWord}).then(res=>{
          switch (res.data){
            case 1:{
              this.$message.success('删除大堂经理成功');
              this.Data.splice(this.Data.indexOf(data),1);
              break;
            }
            case 0:this.$message.error('删除大堂经理失败');break;
          }
        })
      },
      dialogFormYes(){
        this.dialogFormVisible=false;
        this.$http.post('http://localhost:3000/api/L_Manager/Add',
            {Account:this.Register.Account,PassWord:this.Register.PassWord,Name:this.Register.Name,Sex:this.Register.Sex,Phone:this.Register.Phone}).then(res=>{
          switch(res.data){
            case 1:this.$message.success('创建大堂经理成功');break;
            case 0:this.$message.error('创建大堂经理失败');break;}
        })
      }
    },
    watch:{
      Search:{
        ...
        handler(newValue){
          this.currentPage=1
          if(newValue===''){
            this.$http.post('http://localhost:3000/api/L_Manager/Select').then(res=>{
              if(res.data!==0){
                this.Data=res.data
            ...
            this.$http.post('http://localhost:3000/api/L_Manager/SelectManager',{Name:newValue}).then(res=>{
              if(res.data!==0){
                this.Data=res.data
                this.Items=res.data.length
              }
