<template>
  <div>
    <div style="margin-bottom: 5px;margin-top: 5px;">
      <el-input style="width: 93%;" v-model="Search" prefix-icon="el-icon-search" placeholder="请输入服务员姓名">
      </el-input>
    ...
    <el-card v-for="data in Data.slice((currentPage-1)*4,currentPage*4)" :key="data.Account" style="margin-bottom: 5px">
      ...
      <el-popconfirm style="float: right;margin-bottom: 5px;" title="确定删除吗？" @confirm="DeleteWaiter(data)">
...
<script>
  export default {
    ...
    methods:{
      ...
      DeleteWaiter(data){
        this.$http.post('http://localhost:3000/api/Waiter/Delete',
      ...
      dialogFormYes(){
        ...
        this.$http.post('http://localhost:3000/api/Waiter/Add',
          ...
          switch(res.data){
            case 1:this.$message.success('创建服务员成功');break;
            case 0:this.$message.error('创建服务员失败');break;}
        })
      }
    },
    watch:{
      Search:{
        ...
        handler(newValue){
          ...
          this.Data=[]
          if(newValue===''){
            this.$http.post('http://localhost:3000/api/Waiter/Select').then(res=>{
              if(res.data!==0){
                this.Data=res.data
            ...
            this.$http.post('http://localhost:3000/api/Waiter/SelectWaiter',{Name:newValue}).then(res=>{
              if(res.data!==0){
                this.Data=res.data
                this.Items=res.data.length
              }
